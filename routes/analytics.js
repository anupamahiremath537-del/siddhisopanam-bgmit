...
<<<<<<< HEAD
    ...
    regsForCoverage.forEach(r => {
      if (r.type !== 'volunteer') return;
      const key = `${r.eventId}_${r.roleId}`;
...
<<<<<<< HEAD
    ...
    regs.forEach(r => {
      if (!eventRegsMap.has(r.eventId)) eventRegsMap.set(r.eventId, []);
      eventRegsMap.get(r.eventId).push(r);
    });
...
<<<<<<< HEAD
      ...
      volunteers.forEach(v => {
        vByRole.set(v.roleId, (vByRole.get(v.roleId) || 0) + 1);
      });
